            ...
            <div className="max-w-md sm:max-w-lg md:max-w-2xl lg:max-w-5xl xl:max-w-7xl px-2 pb-16 mx-auto">
                <div className="md:grid md:grid-cols-2 gap-5">
                    <div className="mt-6 md:mt-32">
                        ...
                        <p className="mt-3">Versatile transport solutions tailored to your needs. Choose from a range of vehicles including Mazda trucks, containers in various sizes, flatbeds, dumpers, Hyundais and pickups, and efficient railway cargo services for seamless city-to-city delivery.</p>
                        <motion.h3
                            key={vehicle.title}
                            className="text-2xl text-gray-500 font-semibold mt-6"
                            initial={{ y: '30%', opacity: 0 }}
                            animate={{ y: 0, opacity: 1, transition: { duration: 1.2, ease: "easeInOut" } }}
                        >{vehicle.title}</motion.h3>
                        <motion.p
                            className="text-lg mt-4 max-w-md"
                            key={vehicle.description}
                            initial={{ y: '30%', opacity: 0 }}
                            animate={{ y: 0, opacity: 1, transition: { duration: 1.2, ease: "easeInOut" } }}
                        >{vehicle.description}</motion.p>
                    </div>
